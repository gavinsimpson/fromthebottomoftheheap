...
description: >-
  It's fair to say I have gotten a bee^[an entire hive is perhaps more apt!] in my bonnet about
  how palaeolimnologists handle time. For a group of people for whom time is everything, we sure
  do a poor job (in general) of dealing with it in when it comes time to analyse our data. In
  many instances, "poor job" means making no attempt at all to account for the special nature of
  the time series. LOESS comes in for particular criticism because it is widely used by
  palaeolimnologists despite not being particularly suited to the task. Why this is so is perhaps
  due to it's promotion in influential books, papers, and software. I am far from innocent in
  this regard having taught LOESS and it's use for many years on the now-defunct ECRC Numerical
  Course. Here I want to look at further problems with our use of LOESS, and will argue that we
  need to resign it to the trash can for all but exploratory analyses. I will begin the case for
  the prosecution with one of my own transgressions.
...
[Autocorrelation](/tag/autocorrelation/){.badge .text-bg-dark}
[Cross-validation](/tag/cross-validation/){.badge .text-bg-dark}
[GCV](/tag/gcv/){.badge .text-bg-dark}
[LOESS](/tag/loess/){.badge .text-bg-dark}
[Overfitting](/tag/overfitting/){.badge .text-bg-dark}
...
![DCA axis 1 scores for all 19 lakes with diatom paleoecological records. LOESS smooth curve for each park area shows the general trend of diatom community turnover through time. Shaded bars represent the timing of significant shifts in the diatom assemblages (details in the supporting information). Reproduced from Figure 6 from @Hobbs2016-mr.](/assets/img/posts/hobbs-et-al-figure-6.png)
...
![Sediment &delta;^15^N from all cores standardized as z scores. Loess smooth curve in red. Figure 9 from @Hobbs2016-mr.](/assets/img/posts/hobbs-et-al-figure-9.png)
...
<span>[← Older: Soap-film smoothers & lake bathymetries](/2016/03/27/soap-film-smoothers/)</span>
<span class="text-end">[Newer: A new default plot for multivariate dispersions →](/2016/04/17/new-plot-default-for-betadisper/)</span>
:::
