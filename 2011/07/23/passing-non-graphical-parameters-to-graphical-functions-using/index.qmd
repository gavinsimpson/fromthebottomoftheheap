...
tags:
  ...
  - Functions
category: R
excerpt: Argument passing via `...` is a great feature of the R language, allowing
  you to write wrappers around existing functions that do not need to list all the
  arguments of the wrapped function. `...` is used extensively in S3 methods and in
  passing graphical parameters on to graphical functions. When writing you own plot
  methods, using `...` allows the user of your function to pass arguments like `cex`,
  `col`, `lty`, etc. on to the plotting function inside your method. You do, however,
  need to be careful in where you use `...` and which functions you pass `...` on
  to.
date: 2011-07-23 12:00:00
description: >-
  Argument passing via ... is a great feature of the R language, allowing you to write wrappers
  around existing functions that do not need to list all the arguments of the wrapped function.
  ... is used extensively in S3 methods and in passing graphical parameters on to graphical
  functions. When writing you own plot methods, using ... allows the user of your function to
  pass arguments like cex, col, lty, etc. on to the plotting function inside your method. You do,
  however, need to be careful in where you use ... and which functions you pass ... on to.
...
[Programming](/tag/programming/){.badge .text-bg-dark}
[Functions](/tag/functions/){.badge .text-bg-dark}
:::
...
<span>[← Older: Smoothing temporally correlated data](/2011/07/21/smoothing-temporally-correlated-data/)</span>
<span class="text-end">[Newer: permute: a package for generating restricted permutations →](/2011/10/04/permute-a-package-for-generating-restricted-permutations/)</span>
:::
