...
talks <- read_xlsx("../assets/datasets/talk-data.xlsx",
  col_types = c("date", "text", "text", "logical", "logical", "text", "text", "text", "text")) |>
  mutate(year = format(date, "%Y"))

talk_md <- function(title, where, doi, slidedeck, code, recording){
...
recording_md <- ifelse(is.na(recording), "",
    glue("<a href=\"{recording}\"><span class='label label-inverse'>Recording</span></a>"))

glue("
{title}
  ~ {where}
  ~ {doi_md} {slides_md} {code_md} {recording_md}
...
output <- talks |>
  rowwise() |>
  mutate(md = talk_md(title, where, doi, slidedeck, code, recording)) |>
  arrange(desc(date))
