...
The *principal curves* script requires the development version of my [**analogue**](/code/r-packages/analogue/) package. Unfortunately, R-Forge is not building the package at the moment (there is an issue with the installation of the **rgl** package on their build servers). If you are using Linux then you can grab the source code and compile yourself. For MS Windows users, a version that works for the script, has been built using Uwe Ligge's WinBuilder service. Download the [zipfile](/assets/materials/r-packages/binaries/analogue_0.11-6.zip), which was built using R 3.0.2-patched.
