...
#### This file is hosted at: [bit.ly/266nexp21](https://bit.ly/266nexp21)
...
sheets <- excel_sheets(fname) %>%
    set_names()
plants <- map(sheets, read_excel, path = fname)
```
...
If you want formal comparisons of the growth rates for all pairs of treatments, we can also do all pairwise comparisons using the `emmtrends()`function by using the `pairwise` option instead of `trt-vs-ctrl`. Note however, that these results will be the same as the ones above as we have only two groups to compare. As before, we can assess the estimate growth rate (slope) at a given day, here for `day == 5`
...
We don't see any differences in the estimated growth rates at the two days requested, but are there differences in the average plant height between the two treatments? We can test that using the `emmeans()` function, again using the `pairwise` option or `trt-vs-ctrl` because we have only two levels. We use `emmeans()` here because we want estimates of the expected mean height for each treatment at a set of days, and not the slope of a trend line.
...
Note alos that these are on the log-height scale, we could add `transform = "response"` to get the estimates on the original mm height scale

```{r emmeans-pairwise-response-scale}
emmeans(m3, pairwise ~ treatment | day,
        at = list(day = c(1, 5, 10, 15, 20)),
        transform = "response")
```
