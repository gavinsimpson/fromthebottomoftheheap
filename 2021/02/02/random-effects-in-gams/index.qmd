...
tags:
  ...
  - variance components
category: R
image: /assets/img/posts/random-effects-in-gams-plot-m1-gam-predicted-1.png
description: >-
  There are lots of choices for fitting generalized linear mixed effects models within R, but if
  you want to include smooth functions of covariates, the choices are limited. One option is to
  fit the model using gamm() from the mgcv 📦 or gamm4() from the gamm4 📦, which use lme() (nlme
  📦) or one of lmer() or glmer() (lme4 📦) under the hood respectively. The problem with doing
  things that way is that you get PQL fitting for non-Gaussian models (😱) and the range of
  families for handling non-Gaussian responses is quite limited, especially compared with the
  extended families now available with gam(). brms 📦 is a good option if you don't want to do
  everything by hand, but the MCMC can be slow. Instead, we could use the equivalence between
  smooths and random effects and use gam() or bam() from mgcv. In this post I'll show you how to
  do just that.
...
[random effects](/tag/random-effects/){.badge .text-bg-dark}
[smooths](/tag/smooths/){.badge .text-bg-dark}
[penalty](/tag/penalty/){.badge .text-bg-dark}
[variance components](/tag/variance-components/){.badge .text-bg-dark}
:::
...
![Penalty matrix corresponding to a random effect for a factor with 10 subjects (levels).](/assets/img/posts/random-effects-in-gams-penalty-example-1.png)
...
![Plot of the rat hormone therapy data](/assets/img/posts/random-effects-in-gams-plot-rat-data-1.png)
...
![Fitted growth curves from the mixed effect model fitted using `gam()`](/assets/img/posts/random-effects-in-gams-plot-m1-gam-predicted-1.png)
...
![Observed values and fitted growth curves from the mixed effect model fitted using `gam()`](/assets/img/posts/random-effects-in-gams-plot-m1-gam-compare-1.png)
...
![QQ-plot of the rat-specific mean distance effects](/assets/img/posts/random-effects-in-gams-draw-gam-model-1.png)
...
<span>[← Older: Getting data from the Canada Covid-19 Tracker using R](/2021/01/31/getting-data-from-canada-covid-19-tracker-using-r/)</span>
<span class="text-end">[Newer: gratia 0.9.0 →](/2024/03/28/gratia-0-9-0/)</span>
:::
