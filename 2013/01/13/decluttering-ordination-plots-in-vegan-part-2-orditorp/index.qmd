...
description: >-
  In the earlier post in this series") I looked at the ordilabel() function to help tidy up
  ordination biplots in vegan. An alternative function vegan provides is orditorp(), the last
  four letters abbreviating the words text or points. That is a pretty good description of what
  orditorp() does; it draws sample or species labels using text where there is room and where
  there isn't a plotting character is drawn instead. Essentially it boils down to being a one
  stop shop for calls to text() or points() as needed. Let's see how it works...
...
![PCA biplot of the Dutch dune meadow data produced using `orditorp()`](/assets/img/posts/orditorp_figure_combined.png)
...
![PCA species plot of the Dutch dune meadow data produced using `orditorp()` showing the effect of changing argument `air`.](/assets/img/posts/orditorp_figure_air.png)
...
<span>[← Older: Decluttering ordination plots in vegan part 1: ordilabel()](/2013/01/12/decluttering-ordination-plots-in-vegan-part-1-ordilabel/)</span>
<span class="text-end">[Newer: Decluttering ordination plots part 3: ordipointlabel() →](/2013/06/27/decluttering-ordination-plots-in-vegan-part-3-ordipointlabel/)</span>
:::
