...
description: >-
  Quite some years ago, whilst working on the EU Sixth Framework project Euro-limpacs, I
  organized a workshop on statistical methods for analyzing time series data. One of the sessions
  was on the analysis of extremes, ably given by Paul Northrop (UCL Department of Statistical
  Science). That intro certainly whet my appetite but I never quite found the time to dig into
  the arcane world of extreme value theory. Two recent events rekindled my interest in extremes;
  Simon Wood quietly introduced into his mgcv package a family function for the generalized
  extreme value distribution (GEV), and I was asked to review a paper on extremes in time series.
  Since then I've been investigating options for fitting models for extremes to environmental
  time series, especially those that allow for time-varying effects of covariates on the
  parameters of the GEV. One of the first things I did was sit down with mgcv to get a feel for
  the gevlss() family function that Simon had added to the package by repeating an analysis of a
  classic example data set that had been performed using the VGAM package of Thomas Yee.
...
[smoothers](/tag/smoothers/){.badge .text-bg-dark}
[extremes](/tag/extremes/){.badge .text-bg-dark}
[GEV](/tag/gev/){.badge .text-bg-dark}
[extreme values](/tag/extreme-values/){.badge .text-bg-dark}
...
![Time series of annual sea-level maxima at Fremantle, Western Australia (top) and the relationship between annual sea-level maxima and the Southern Oscillation Index](/assets/img/posts/modelling-extremes-with-gams-post-1-packages-and-plot-1.png)
...
![Fitted smooths for model `m1` which uses penalized splines for the smooths of `Year` and `SOI` in the linear predictors for the location and scale parameters of the GEV distribution](/assets/img/posts/modelling-extremes-with-gams-post-1-plot-m1-smooths-1.png)
...
![Fitted smooths and parametric terms for model `m2` which uses a piece-wise linear spline for the effect of `Year` on the scale parameter](/assets/img/posts/modelling-extremes-with-gams-post-1-plot-m2-smooths-1.png)
...
![Fitted smooths and parametric terms for model `m3` which uses an adaptive spline for the effect of `Year` on the scale parameter](/assets/img/posts/modelling-extremes-with-gams-post-1-plot-m3-smooths-1.png)
...
<span>[← Older: Pangaea and R and open palaeo data](/2016/12/16/pangaea-r-open-palaeo-data/)</span>
<span class="text-end">[Newer: Simultaneous intervals for derivatives of smooths revisited →](/2017/03/21/simultaneous-intervals-for-derivatives-of-smooths/)</span>
:::
