...
```{r gam-plot, cache = FALSE, fig.cap = "The fitted penalised spline with approximate 95% point-wise confidence interval, as produced with `plot.gam()`"}
...
The critical value estimated above is `r round(crit, 3)`. Intervals generated using this value will be `r round(crit / 2, 2)` times larger than the point-wise interval shown above.
...
```{r confidence-intervals, cache = FALSE, fig.cap = "Comparison of point-wise and simultaneous 95% confidence intervals for the fitted GAM"}
...
`fits` now contains N = `r N` draws from the model posterior. Before we look at how many of the `r N` samples from the posterior are entirely contained within the simultaneous interval, choose 30 at random and stack them in so-called tidy form for use with `ggplot()`
...
```{r plot-intervals-and-posterior-draws, cache = FALSE, fig.cap = "Summary plot showing 30 random draws from the model posterior and approximate 95% simultaneous and point-wise confidence intervals for the the fitted GAM"}
...
```{r coverage-properties, cache = FALSE}
