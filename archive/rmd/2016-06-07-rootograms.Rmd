...
```{r initial-pkgs, echo = FALSE, results = "hide", message = FALSE, cache = FALSE}
library("cowplot")
theme_set(theme_bw())
```
...
```{r simulate-setup, message = FALSE}
library("coenocliner")
...
Now we can simulate counts for the `r length(locs)` locations along the gradient for each of the three count models

```{r simulate, dependson = "simulate-setup"}
...
```{r load-countreg, cache = FALSE, message = FALSE}
library("countreg")
library("ggplot2")
```
...
```{r plot-rootograms, plot = TRUE, fig.height = 3.33, fig.width = 10, cache = FALSE, fig.cap = "Hanging rootograms for a Poisson GLM fitted to simulated Poisson (a), negative binomial (b), and zero-inflated negative binomial (c) count data"}
...
```{r plot-negbin-rootos, cache = FALSE, fig.cap = "Hanging rootograms for Poisson GLM (a) and negative binomial model (b) fits to the simulated negative binomial count data"}
...
```{r plot-zinb-rootos, cache = FALSE, fig.cap = "Hanging rootograms for Poisson GLM (a) and zero-inflated negative binomial model (b) fits to the simulated zero-inflated negative binomial count data"}
ylims <- ylim(-2, 8.5)
...
```{r plot-zinb-suspended, cache = FALSE, fig.height = 7, fig.width = 7, fig.cap = "Suspended rootogram for a zero-inflated negative binomial model fitted to the simulated zero-inflated negative binomial count data"}
...
```{r plot-zinb-standing, cache = FALSE, fig.height = 7, fig.width = 7, fig.cap = "Standing rootogram for a zero-inflated negative binomial model fitted to the simulated zero-inflated negative binomial count data"}
...
```{r plot-concatenated-rootograms, cache = FALSE, fig.height = 7, fig.width = 7, fig.cap = "Result of plotting two rootograms that were combined using `cbind()`"}
