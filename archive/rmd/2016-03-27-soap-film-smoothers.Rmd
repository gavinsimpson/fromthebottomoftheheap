...
```{r fs-boundary-figure, fig.cap = "Ramsay's test function", fig.height = 5, message = FALSE, warning = FALSE}
library("mgcv")
fsb <- fs.boundary()
m <- 300
n <- 150 
...
```{r show-comeston-lakes-baty-data, fig.cap = "Comeston Park Lakes depth sounding data", fig.height = 5, message = FALSE, results = "hide"}
library("rgdal")
...
```{r plot-tprs-model-fit, fig.cap = "Predicted depths over the bounding box of the observations from the TPRS smoother GAM.", fig.height = 5, warning = FALSE}
ggplot(foutline, aes(x = long, y = lat)) +
...
```{r plot-soap-film-set-up, fig.cap = "Illustration of the knot selection procedure. The large circles are the locations of the sparse regular grid of points over the bounding box of the data. The filled red circles are those grid points that are found inside the lake boundary and thus chosen as knots for the soap-film smoother. The small black dots are the locations of the observed depth data.", echo = FALSE, fig.height = 5}
...
```{r soap-film-plot-method, fig.cap = "Contour plot of the fitted sop-film spline produced using `plot.gam()` with `scheme = 2`.", fig.height = 5}
...
```{r ggplot-soap-film, fig.cap = "The fitted surface achieved using a soap-film smoother", fig.height = 5, warning = FALSE}
...
```{r combined-plot, fig.cap = "Comparison of fitted depth surfaces for the soap-film and TPRS smoother models", fig.height = 5}
