...
tags:
  ...
  - penalty
category: R
image: /assets/img/posts/extrapolating-with-b-splines-and-gams-plot-b-spline-with-mixed-penalties-1.png
description: >-
  An issue that often crops up when modelling with generlaized additive models (GAMs), especially
  with time series or spatial data, is how to extrapolate beyond the range of the data used to
  train the model? The issue arises because GAMs use splines to learn from the data using basis
  functions. The splines themselves are built from basis functions that are typically setup in
  terms of the data used to fit the model. If there are no basis functions beyond the range of
  the input data, what exactly is being used if we want to extrapolate? A related issue is that
  of the wiggliness penalty; depending on the type of basis used, the penalty could extend over
  the entire real line (-&#8734;&ndash;&#8734;), or only over the range of the input data. In
  this post I want to take a practical look the extrapolation behaviour of splines in GAMs fitted
  with the mgcv package for R. In particular I want to illustrate how flexible the B spline basis
  is.
...
[splines](/tag/splines/){.badge .text-bg-dark}
[B splines](/tag/b-splines/){.badge .text-bg-dark}
[mgcv](/tag/mgcv/){.badge .text-bg-dark}
[extrapolation](/tag/extrapolation/){.badge .text-bg-dark}
[penalty](/tag/penalty/){.badge .text-bg-dark}
:::
...
![Posterior predictive means for the two thin plate regression spline models showing the interpolation and extrapolation behaviour with first and second derivative penalties.](/assets/img/posts/extrapolating-with-b-splines-and-gams-plot-tprs-predictions-1.svg)
...
![Posterior predictive means for the two B spline models showing the interpolation and extrapolation behaviour when the penalty over covers the range of the data and when it extends byond that range.](/assets/img/posts/extrapolating-with-b-splines-and-gams-plot-bs-eg-predictions-1.svg)
...
```
![Posterior predictive means for three GAMs using B splines with mixtures of derivative penalties, all covering the region where we want to predict for the test samples; a B spline with single 2nd derivative penalty, a B spline with a 2nd and 1st derivative penalties, and a B spline with 2^nd^, 1^st^ and 0^th^ derivative penalties.](/assets/img/posts/extrapolating-with-b-splines-and-gams-plot-b-spline-with-mixed-penalties-1.svg)
...
<span>[← Older: gratia 0.4.1 released](/2020/05/31/new-gratia-release/)</span>
<span class="text-end">[Newer: Two new versions of gratia released →](/2021/01/30/two-new-versions-of-gratia-on-cran/)</span>
:::
...
```{r plot-mixed-spline-predictions}
#| eval: false
#| echo: false
#| output: false
#| include: false
...
```{r fit-b-spline-models-with-diff-penalties}
#| eval: false
#| echo: false
#| output: false
#| include: false
...
```{r plot-b-spline-diff-penalties}
#| eval: false
#| echo: false
#| output: false
#| include: false
...
```{r plot-b-spline-with-mixed-penalties}
#| eval: false
#| echo: false
#| output: false
#| include: false
