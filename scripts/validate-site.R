...
if (!grepl("#quarto-document-content .column-margin .post-taxonomy", theme_text, fixed = TRUE) ||
    !grepl("grid-template-columns: minmax(0, 1fr) 150px", theme_text, fixed = TRUE) ||
...
publication_navigation <- regexpr('class="publication-navigation"', main_publications, fixed = TRUE)[[1L]]
publication_selector <- regexpr('class="publication-year-selector dropdown side-snippet"', main_publications, fixed = TRUE)[[1L]]
publication_total <- regexpr('class="publication-total"', main_publications, fixed = TRUE)[[1L]]
publication_unpublished <- regexpr('href="#unpublished">Unpublished (', main_publications, fixed = TRUE)[[1L]]
...
if (any(c(publication_navigation, publication_selector, publication_total, publication_unpublished,
          publication_social_wrapper, publication_social) < 1L) ||
    !(publication_navigation <= publication_total && publication_total < publication_selector &&
      publication_selector < publication_social_wrapper &&
      publication_social_wrapper < publication_social) ||
    grepl("Current work", main_publications, fixed = TRUE) ||
    grepl("icon have a preprint or reprint", main_publications, fixed = TRUE) ||
