...
clean_body <- function(lines) {
  ...
  text <- gsub("\\{%\\s*include\\s+cc-by-icon\\.html\\s*%\\}", '<span class="publication-licence publication-licence-cc-by" role="img" aria-label="Creative Commons Attribution">{{< fa brands creative-commons >}}{{< fa brands creative-commons-by >}}</span>', text, perl = TRUE)
  text <- gsub("\\{%\\s*include\\s+cc-by-nc-icon\\.html\\s*%\\}", '<span class="publication-licence publication-licence-cc-by-nc" role="img" aria-label="Creative Commons Attribution-NonCommercial">{{< fa brands creative-commons >}}{{< fa brands creative-commons-by >}}{{< fa brands creative-commons-nc >}}</span>', text, perl = TRUE)
