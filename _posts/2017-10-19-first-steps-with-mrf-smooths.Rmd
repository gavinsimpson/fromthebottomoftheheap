...
```{r packages, include = FALSE, message = FALSE, warning = FALSE, cache = FALSE}
library('rgdal')
library('proj4')
library('spdep')
library('mgcv')
...
```{r load-shapefile, include = FALSE, message = FALSE, warning = FALSE}
...
```{r ggplot-stuff, include = FALSE, message = FALSE, warning = FALSE}
theme_map <- function(...) {
    theme_minimal() +
    theme(...,
...
```{r hsd-data-plot, echo = FALSE, fig.cap = '', fig.lp = '', fig.width = 10, fig.height = 7.5, cache = TRUE, depends.on = 'ggplot-stuff', warning = FALSE}
...
```{r load-packages, ref.label = 'packages', cache = FALSE}
```
...
```{r data-prep, ref.label = 'prepare-data', eval = FALSE}
```
...
```{r fit-reduced-rank-models, eval = FALSE}
## rank 300 MRF
...
```{r ggplot-constants, ref.label = 'ggplot-stuff'}
```
...
```{r plot-full-rank-mrf, fig.cap = '', fig.lp = '', fig.width = 10, fig.height = 7.5, depends.on = 'ggplot-stuff'}
...
```{r plot-rank-300-mrf, fig.cap = '', fig.lp = '', fig.width = 10, fig.height = 7.5, depends.on = 'ggplot-stuff'}
ggplot(mdata, aes(x = long, y = lat, group = group)) +
...
```{r plot-rank-30-mrf, fig.cap = '', fig.lp = '', fig.width = 10, fig.height = 7.5, depends.on = 'ggplot-stuff'}
ggplot(mdata, aes(x = long, y = lat, group = group)) +
